...
public class TaskController {
    ...
    @GetMapping("/tasks")
    public String taskPage(@RequestParam(name = "page", defaultValue = "1") int page,
                           Principal principal, Model model) {
        int safePage = Math.max(1, page);

        long totalCount = taskService.countMyTasks(principal.getName());
        ...
        List<Task> tasks = taskService.findMyTasks(principal.getName(), safePage - 1, PAGE_SIZE);
    ...
    public String update(@PathVariable("id") Long id, @Valid @ModelAttribute("form") TaskForm form,
